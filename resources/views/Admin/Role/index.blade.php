                                ...
                                @if($data->count() > 0)
                                    @foreach($data as $record)
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    $permissions = json_decode($record->permissions) == '' ? [] : json_decode($record->permissions);
                                                @endphp
                                                @if(count($permissions) > 0)
                                                    <ul class="perm-list p-l-hide">
                                                        @foreach($permissions as $permission)
                                                        <li>{{ ucwords(str_replace('_', ' ', $permission)) }}</li>
                                                        @endforeach
                                                    </ul>
                                                    @if(count(json_decode($record->permissions)) > 3)
                                                        <a href="javascript:;" class="s-m-perm">Show More</a>
                                                    @endif
                                                @endif
